<template>
  <div class="home">
                  ...
                  <v-form v-model="insertFormValid">
                    <v-row>
                      <v-col cols="12" sm="6" md="4">
                        <v-text-field v-model="insertForm.new_aid" :rules="[rules.required, rules.number]" label="Artist ID"></v-text-field>
                      </v-col>
                      <v-col cols="12" sm="6" md="4">
                        <v-text-field v-model="insertForm.new_aname" :rules="[rules.required]" label="Artist Name"></v-text-field>
                      </v-col>
                    </v-row>
                  </v-form>
                </v-container>
          ...
          <v-card-text>
            <v-container>
                <v-row>
                  ...
                  <v-col cols="12" sm="6">
                    <v-text-field v-model="searchForm.new_aid" label="Artist ID"></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6">
                    <v-text-field v-model="searchForm.new_aname" label="Artist Name"></v-text-field>
                  </v-col>
                </v-row>
    ...
    <v-dialog v-model="confirmDeleteDialog" max-width="600px">
      <v-card>
        <v-card-title class="headline justify-center">Are you sure you want to delete this artist?</v-card-title>
        <v-card-actions>
          <v-spacer></v-spacer>
        ...
        <v-form v-model="editFormValid" @submit.prevent="edit">
          ...
          <v-card-text>
            <v-container>
                <v-row>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editObject.artist_id" :disabled="true" label="Artist ID"></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editObject.artist_name" :disabled="false" label="Artist Name"></v-text-field>
                  </v-col>
                </v-row>
            </v-container>
          </v-card-text>
...
export default {
  name: "Artists", // name of this particular web page
  ...
  methods: { // "methods" is an object of functions that the webpage can/will run
    search() { // Could be used to search a table by sending a post request
      ...
      this.searchEdited = (this.searchForm.new_aid.length > 0 ? true : this.searchEdited)
      this.searchEdited = (this.searchForm.new_aname.length > 0 ? true : this.searchEdited)
      // console.log(this.searchForm.new_id.length > 0)
      this.emptyForm = !this.searchEdited
      axios
        .post("https://mood.leinad.pw/artists/search", {search: this.searchForm, edited: this.searchEdited})
        .then((response) => {
    ...
    insert() { // Could be used to insert into the table using a put request
      ...
      axios
        .post("https://mood.leinad.pw/artists/insert", {insert: this.insertForm})
        .then((response) => {
            console.log(response)
        })
        .catch((error) => {
    ...
    edit(item) { // Could be used to edit a table entry using a post request
        console.log(item)
        this.editDialog = false
        axios
          .post("https://mood.leinad.pw/artists/edit", {edit: this.editObject})
          .then((response) => {
              console.log(response)
          })
          .catch((error) => {
    ...
    confirmDelete() { // Could be used to delete a table entry using a delete request
        ...
        axios
          .delete("https://mood.leinad.pw/artists/delete", {data: {artist_id: this.deleteObject.artist_id, artist_name: this.deleteObject.artist_name}})
          .then((response) => {
              console.log(response);
              this.search();
          })
