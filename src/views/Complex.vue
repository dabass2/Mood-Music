...
export default {
  ...
  methods: { // "methods" is an object of functions that the webpage can/will run
    search1() { // Could be used to search a table by sending a post request
      ...
      axios
        .get(`http://localhost:9001/complex/1`, {search: this.searchForm, edited: this.searchEdited})
        .then((response) => {
    ...
    search2() { // Could be used to search a table by sending a post request
      ...
      axios
        .get(`http://localhost:9001/complex/2`, {search: this.searchForm, edited: this.searchEdited})
        .then((response) => {
            console.log(response)
            this.tableData = response.data.results
            this.dataLength = response.data.results.length
    ...
    search3() { // Could be used to search a table by sending a post request
      ...
      axios
        .get(`http://localhost:9001/complex/3`, {search: this.searchForm, edited: this.searchEdited})
        .then((response) => {
            console.log(response)
            this.tableData = response.data.results
            this.dataLength = response.data.results.length
    ...
    search4() { // Could be used to search a table by sending a post request
      ...
      axios
        .get(`http://localhost:9001/complex/4`, {search: this.searchForm, edited: this.searchEdited})
        .then((response) => {
            console.log(response)
            this.tableData = response.data.results
            this.dataLength = response.data.results.length
